...
def query_pinecone_law(text: str, top_k: int):
    try:
        text_vector = text_embedding(text)
        index = pinecone.Index("assembly-law")
        query_res = index.query(
            vector=text_vector,
            top_k=top_k,
            include_values=False
...
def search_law_by_id(id: int):
    fetch = supabase.table('law').select('*').eq('pinecone_id', id).execute()
    res_json = json.loads(fetch.json())['data']
    return res_json[0] if len(res_json) > 0 else None